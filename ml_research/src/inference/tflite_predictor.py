"""Inference helper for TensorFlow Lite model."""
...
class TFLitePredictor:
    """Predictor for .tflite binary HELP detector."""
    ...
    def preprocess(self, wav_path: str | Path) -> np.ndarray:
        """Load WAV and transform to model input tensor."""
        y = load_audio(
            wav_path,
    ...
    def predict(self, wav_path: str | Path) -> Dict[str, float | str]:
        """Run TFLite inference and return probability dictionary."""
        x = self.preprocess(wav_path)
        self.interpreter.set_tensor(self.input_details[0]["index"], x)
        self.interpreter.invoke()
        out = self.interpreter.get_tensor(self.output_details[0]["index"])
        prob_help = float(np.array(out).reshape(-1)[0])
